...
```{r maPreproc}
geo2kegg <- maPreprocApply(geo2kegg[1:5])
```
...
Note: The `maPreprocApply` returns datasets consistently mapped to NCBI Entrez
...
done using the function `EnrichmentBrowser::map.ids`.  
...
```{r deAna}
geo2kegg <- deApply(geo2kegg, de.method="limma", padj.method="flexible")
...
```{r runEA}
kegg.ora.res <- runEA(geo2kegg[[1]], method="ora", gs=kegg.gs)
kegg.ora.res
```
...
```{r eaAll}
res.dir <- tempdir()
res <- eaApply(geo2kegg, methods=c("ora", "camera"), 
...
```{r readRT}
ea.rtimes <- readRuntime(res.dir, names(geo2kegg), methods=c("ora", "camera"))
ea.rtimes
```
...
```{r readRankings}
ea.ranks <- readRankings(res.dir, names(geo2kegg), methods=c("ora", "camera"))
lengths(ea.ranks)
ea.ranks$ora[1:2]
```
...
`compRand` repeatedly applies `evalRanks` to randomly drawn gene set rankings. 
...
all.kegg.res <- evalApply(ea.ranks, mala.kegg, d2d.map[names(geo2kegg)])
...
of GSEABenchmarkeR's *Apply*-functions, i.e. `maPreprocApply`, `deApply`, and
especially `eaApply`, is straightforward.
...
```
shows that the execution uses a `MulticoreParam` per default (on Windows: a `SnowParam`), 
where the `bpnworkers` attribute indicates the number of cores involved in the computation.
...
```{r deApplyBP}
geo2kegg <- deApply(geo2kegg, parallel=bp.par)
```
